...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  let workingDaysCounter = countWorkDays;
  let daysOffCounter = countOffDays;
  const resultSchedule = [];
  while (currentDate <= endDate) {
    if (workingDaysCounter > 0) {
      const day = String(currentDate.getDate()).padStart(2, '0');
      const month = String(currentDate.getMonth() + 1).padStart(2, '0');
      const year = currentDate.getFullYear();
      resultSchedule.push(`${day}-${month}-${year}`);
      workingDaysCounter -= 1;
    } else {
      daysOffCounter -= 1;
    }

    if (workingDaysCounter === 0 && daysOffCounter === 0) {
      workingDaysCounter = countWorkDays;
      daysOffCounter = countOffDays;
    }
